...
class KioskCommissionReportController extends Controller
{
    ...
    private const TAB_KEY = 'commission';
    ...
    public function export(Request $request): JsonResponse|StreamedResponse|Response
    {
        ...
        if ($format === 'pdf') {
            ...
            return Pdf::loadView('reports.table', [
                ...
                'truncated' => $truncated,
            ])->setPaper('a4', 'landscape')->download('kiosk-commission-report-'.now()->format('Ymd-His').'.pdf');
        }

        $filename = 'kiosk-commission-report-'.now()->format('Ymd-His').'.csv';
